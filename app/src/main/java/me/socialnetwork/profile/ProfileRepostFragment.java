...
public class ProfileRepostFragment extends ProfileTabFragment {

    public ProfileRepostFragment(String userId) {
        super(userId);
    }
    ...
    @Override
    void request() {
        StandardAPI.getService.getProfilePosts(cookie, requestBody, "reposts", postList.size()).enqueue(new Callback<ArrayList<Post>>() {
            @SuppressLint("NotifyDataSetChanged")
            @Override
            public void onResponse(@NonNull Call<ArrayList<Post>> call, @NonNull Response<ArrayList<Post>> response) {
                if (!response.isSuccessful()) return;
                postList.addAll(response.body());
                title.setText(!postList.isEmpty() ? "" : "Không có bài đăng lại nào.");
